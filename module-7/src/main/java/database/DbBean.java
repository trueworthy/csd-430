...
public class DbBean implements java.io.Serializable {
	...
	// Read the most recently added book
	public String readLast() {
		StringBuilder sb = new StringBuilder();
		try {
			ResultSet rs = statement.executeQuery("SELECT * FROM lea_library_data ORDER BY book_id DESC LIMIT 1");
			while (rs.next()) {
				sb.append("<tr>").append("<td>").append(rs.getInt("book_id")).append("</td>").append("<td>")
						.append(rs.getString("title")).append("</td>").append("<td>").append(rs.getString("author"))
						.append("</td>").append("<td>").append(rs.getInt("publish_year")).append("</td>").append("<td>")
			...
			return "<tr><td colspan='5'>Error reading latest book: " + e.getMessage() + "</td></tr>";
		}
		return sb.toString();
	}
	...
	// Generate <option> list for book IDs
	...
	// Provide the PK form for GET requests
